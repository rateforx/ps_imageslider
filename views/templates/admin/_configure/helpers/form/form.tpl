                        ...
                        {if isset($fields[0]['form']['images'])}
                            {if $input.name == 'image'}
                                <img class = "img-thumbnail"
                                     src = "{$image_baseurl}{$fields[0]['form']['images'][$language.id_lang]}"
                                />
                            {/if}
                            {if $input.name == 'imageMobile'}
                                <img class = "img-thumbnail"
                                     src = "{$image_baseurl}{$fields[0]['form']['imagesMobile'][$language.id_lang]}"
                                />
                            {/if}
                        {/if}
                        <div class = "dummyfile input-group">
                            <input id = "{$input.name}_{$language.id_lang}" type = "file"
                                   name = "{$input.name}_{$language.id_lang}" class = "hide-file-upload" />
