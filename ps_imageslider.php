...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    protected function installSamples ()
    {
        ...
        for ( $i = 1; $i <= 3; ++$i ) {
            ...
            foreach ( $languages as $language ) {
                ...
                $slide->imageMobile[ $language[ 'id_lang' ] ] = 'sample-' . $i . '.jpg';
            }
            $slide->add();
        }
    }
    ...
    protected function _postValidation ()
    {
        ...
        if ( Tools::isSubmit( 'submitSlider' ) ) {
            ...
            foreach ( $languages as $language ) {
                ...
                if ( Tools::getValue( 'imageMobile_' . $language[ 'id_lang' ] ) != null && !Validate::isFileName( Tools::getValue( 'imageMobile_' . $language[ 'id_lang' ] ) ) ) {
                    $errors[] = $this->getTranslator()->trans( 'Invalid filename.', array(), 'Modules.Imageslider.Admin' );
                }
                if ( Tools::getValue( 'imageMobile_old_' . $language[ 'id_lang' ] ) != null && !Validate::isFileName( Tools::getValue( 'imageMobile_old_' . $language[ 'id_lang' ] ) ) ) {
                    $errors[] = $this->getTranslator()->trans( 'Invalid filename.', array(), 'Modules.Imageslider.Admin' );
                }
            }
            ...
            if ( !Tools::isSubmit( 'has_pictureMobile' ) && ( !isset( $_FILES[ 'imageMobile_' . $id_lang_default ] ) || empty( $_FILES[ 'imageMobile_' . $id_lang_default ][ 'tmp_name' ] ) ) ) {
                $errors[] = $this->getTranslator()->trans( 'The image is not set.', array(), 'Modules.Imageslider.Admin' );
            }
            if ( Tools::getValue( 'imageMobile_old_' . $id_lang_default ) && !Validate::isFileName( Tools::getValue( 'imageMobile_old_' . $id_lang_default ) ) ) {
                $errors[] = $this->getTranslator()->trans( 'The image is not set.', array(), 'Modules.Imageslider.Admin' );
            }
    ...
    protected function _postProcess ()
    {
        ...
        if ( Tools::isSubmit( 'submitSlider' ) ) {
            ...
            foreach ( $languages as $language ) {
                ...
                $typeMobile = Tools::strtolower( Tools::substr( strrchr( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ][ 'name' ], '.' ), 1 ) );

                //mobile
                $imagesize = @getimagesize( $_FILES[ 'image_' . $language[ 'id_lang' ] ][ 'tmp_name' ] );
                $imagesizeMobile = @getimagesize( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ][ 'tmp_name' ] );
                if ( isset( $_FILES[ 'image_' . $language[ 'id_lang' ] ] ) &&
                    ...
                    //mobile check
                    isset( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ] ) &&
                    isset( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ][ 'tmp_name' ] ] ) &&
                    !empty( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ][ 'tmp_name' ] ] ) &&
                    !empty( $imagesizeMobile ) &&
                    in_array(
                        Tools::strtolower( Tools::substr( strrchr( $imagesizeMobile[ 'mime' ], '/' ), 1 ) ), array(
                            'jpg',
                            'gif',
                            'jpeg',
                            'png'
                    ...
                    in_array( $typeMobile, array( 'jpg', 'gif', 'jpeg', 'png' ) )
                ) {
                    ...
                    //mobile
                    if ( $error = ImageManager::validateUpload( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ] ) ) {
                        $errors[] = $error;
                    } elseif ( !$temp_name || !move_uploaded_file( $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ][ 'tmp_name' ], $temp_name ) ) {
                        return false;
                    } elseif ( !ImageManager::resize( $temp_name, __DIR__ . '/images/mobile-' . $salt . '_' . $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ][ 'name' ], null, null, $type ) ) {
                        $errors[] = $this->displayError( $this->getTranslator()->trans( 'An error occurred during the image upload process.', array(), 'Admin.Notifications.Error' ) );
                    }
                    if ( isset( $temp_name ) ) {
                        @unlink( $temp_name );
                    }
                    $slide->imageMobile[ $language[ 'id_lang' ] ] = 'mobile-' . $salt . '_' . $_FILES[ 'imageMobile_' . $language[ 'id_lang' ] ][ 'name' ];

                } elseif ( Tools::getValue( 'imageMobile_old_' . $language[ 'id_lang' ] ) != '' ) {
    ...
    public function getSlides ( $active = null )
    {
        ...
        $slides = Db::getInstance( _PS_USE_SQL_SLAVE_ )->executeS( '
            ...
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`, hssl.`imageMobile`
        ...
        foreach ( $slides as &$slide ) {
            ...
            $slide[ 'image_url_mobile' ] = $this->context->link->getMediaLink( _MODULE_DIR_ . 'ps_imageslider/images/' . $slide[ 'imageMobile' ] );
    ...
    public function getAllImagesBySlidesId ( $id_slides, $active = null, $id_shop = null )
    {
        ...
        $results = Db::getInstance( _PS_USE_SQL_SLAVE_ )->executeS( '
            SELECT hssl.`image`, hssl.`imageMobile`, hssl.`id_lang`
            FROM ' . _DB_PREFIX_ . 'homeslider hs
            LEFT JOIN ' . _DB_PREFIX_ . 'homeslider_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
            LEFT JOIN ' . _DB_PREFIX_ . 'homeslider_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)
    ...
    public function renderAddForm ()
    {
        ...
        if ( Tools::isSubmit( 'id_slide' ) && $this->slideExists( (int)Tools::getValue( 'id_slide' ) ) ) {
            ...
            $fields_form[ 'form' ][ 'imagesMobile' ] = $slide->imageMobile;
    ...
    public function getAddFieldsValues ()
    {
        ...
        foreach ( $languages as $lang ) {
            ...
            $fields[ 'imageMobile' ][ $lang[ 'id_lang' ] ] = Tools::getValue( 'imageMobile_' . (int)$lang[ 'id_lang' ] );
